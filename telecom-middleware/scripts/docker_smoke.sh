...
IMAGE="${1:-telecom-middleware:local}"
...
docker run -d --name "$NAME" -p "${PORT}:9000" \
  -e TELECOM_MW_ENV=local \
  -e TELECOM_MW_HTTP_HOST=0.0.0.0 \
  -e TELECOM_MW_LOCAL_VERIFIER_SECRET=smoke-test-secret-long-enough-for-hs256 \
  "$IMAGE" >/dev/null
