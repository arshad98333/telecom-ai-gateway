...
# The dataset itself — customer profiles, plans, seeds, reasons and cases — lives in
# seed_bulk_data.py, next to this file. This module is only the orchestration: how the
# dataset gets loaded and reported, not what it contains. A plain sibling import, not a
# `scripts.` package import: this file is run directly (`python scripts/seed_bulk.py`),
# which puts `scripts/` itself, not its parent, on `sys.path`.
