...
def test_an_explicit_mapping_ignores_the_ambient_environment(
    ...
    source runs even under ``model_validate``, so an exported ``TELECOM_MW_`` variable
    silently overrides a default the caller never asked to change. The CI job that stands
    up a replica set exports TELECOM_MW_STORE, and six tests asserting the safe defaults
    saw a configured backend instead. This is that regression.
    """
    monkeypatch.setenv("TELECOM_MW_STORE", "mongodb")
    monkeypatch.setenv("TELECOM_MW_ENV", "production")
