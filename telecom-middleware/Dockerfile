...
# substitution is a copy, not a hunt. Tracked in ../telecom-mcp/docs/decisions/0004-container-base.md.
...
RUN groupadd --system --gid 10002 telecom \
 && useradd --system --uid 10002 --gid telecom --no-create-home --shell /usr/sbin/nologin telecom \
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=10s --retries=3 \
    CMD ["python", "-c", "import sys,urllib.request;sys.exit(0 if urllib.request.urlopen('http://127.0.0.1:9000/readyz', timeout=2).status==200 else 1)"]

ENTRYPOINT ["telecom-middleware"]
CMD ["serve"]
