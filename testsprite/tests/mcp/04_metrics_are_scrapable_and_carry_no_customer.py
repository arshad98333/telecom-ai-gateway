"""A metric label that identifies a customer is an incident."""
...
def test_metrics_render_prometheus_without_identifying_anybody() -> None:
    response = requests.get(f"{BASE_URL}/metrics", timeout=TIMEOUT)
    assert response.status_code == 200, response.text
    assert "text/plain" in response.headers.get("content-type", ""), response.headers

    text = response.text
    # The registry restricts labels to an allow-list. Nothing identifying may appear.
    for forbidden in ("cx_id=", "subject=", "msisdn=", "email=", "token="):
        assert forbidden not in text, f"{forbidden} appears as a metric label"

    # Either the process has served traffic, in which case the series exist, or it has
    # not, in which case an empty body is correct. Both are fine; a 500 is not.
    if text.strip():
        assert "# TYPE" in text, text[:300]


test_metrics_render_prometheus_without_identifying_anybody()
