"""The control that matters most: one customer cannot read another."""
...
def test_asking_about_another_customer_is_refused() -> None:
    mine = subject()
    other = "CX-000000-NOT-MINE" if mine != "CX-000000-NOT-MINE" else "CX-111111-NOT-MINE"

    text = call_tool("get_invoice_summary", {"cx_id": other, "limit": 2})

    assert "not permitted" in text, f"a cross-account read was not refused: {text[:400]}"
    # The refusal must not reveal whether that account exists; the wording is identical
    # either way, so a prober cannot enumerate accounts by reading the difference.
    assert "not found" not in text.lower(), (
        f"the refusal leaks whether the account exists: {text[:400]}"
    )


def test_my_own_account_still_works() -> None:
    # Without this, a server that refused absolutely everything would pass the test above.
    text = call_tool("get_invoice_summary", {"cx_id": subject(), "limit": 1}, request_id=10)
    assert "invoices" in text, f"the control refuses the owner too: {text[:400]}"


test_asking_about_another_customer_is_refused()
test_my_own_account_still_works()
