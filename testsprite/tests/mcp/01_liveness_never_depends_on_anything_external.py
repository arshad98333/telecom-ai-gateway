"""Liveness answers even when the world outside is broken."""
...
def test_liveness_answers_healthy_with_a_version() -> None:
    response = requests.get(f"{BASE_URL}/healthz", timeout=TIMEOUT)
    assert response.status_code == 200, response.text
    body = response.json()
    assert body["status"] == "healthy", body
    # Liveness must consult nothing external. A component list here would mean a
    # dependency outage could restart every replica at once.
    assert body["components"] == [], body
    assert body["version"], "the probe must say which build answered"


test_liveness_answers_healthy_with_a_version()
