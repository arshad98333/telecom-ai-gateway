"""The restricted action, and the promise it makes."""
...
def test_a_refund_request_is_queued_for_a_human_and_moves_nothing() -> None:
    text = call_tool(
        "request_refund_approval",
        {
            ...
            "justification": "Charged twice for the same month; the duplicate is on INV-0001.",
            "idempotency_key": "testsprite-refund-0001",
        },
        request_id=50,
    )
    ...
    if "not permitted" in text:
        ...
        # not have been listed either. The refusal being clean is all there is to assert.
        assert "safety control" not in text, text[:400]
        return

    payload = json.loads(text)
    assert payload["state"] == "pending_approval", payload
    assert payload["approver_role"] == "supervisor_approver", payload
    # The whole point of this tool: it queues a decision, it does not make one.
    assert payload["money_moved"] is False, f"the tool claims money moved: {payload}"
    assert payload.get("approval_request_id"), payload


test_a_refund_request_is_queued_for_a_human_and_moves_nothing()
