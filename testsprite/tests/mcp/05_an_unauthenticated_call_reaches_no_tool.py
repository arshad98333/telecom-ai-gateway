"""No token, no tools. The first stage of the kernel."""
...
def test_a_call_without_a_bearer_token_is_refused() -> None:
    answer = rpc(
        "tools/call",
        {"name": "get_invoice_summary", "arguments": {"cx_id": "CX-1234", "limit": 2}},
        request_id=1,
        authenticated=False,
    )
    # Either the transport rejects it outright, or the kernel refuses the call. Both are
    # correct; silently answering is not.
    if answer["status"] == 200:
        content = answer["body"].get("result", {}).get("content")
        assert content, f"an unauthenticated call was answered with no refusal: {answer['body']}"
        text = content[0]["text"]
        assert "Authentication is required" in text or "not valid" in text, text
    else:
        assert answer["status"] in (400, 401, 403), answer


test_a_call_without_a_bearer_token_is_refused()
