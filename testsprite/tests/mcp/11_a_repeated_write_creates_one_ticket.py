"""The same idempotency key twice must not create two tickets."""
...
ARGUMENTS = {
    ...
    "description": "I was billed twice for the same month and would like it looked at.",
    "idempotency_key": "testsprite-idempotency-0001",
}


def test_repeating_an_identical_write_returns_the_original() -> None:
    arguments = dict(ARGUMENTS, cx_id=subject())

    first = json.loads(call_tool("create_support_ticket", arguments, request_id=40))
    assert first.get("ticket_id"), first

    second = json.loads(call_tool("create_support_ticket", arguments, request_id=41))
    assert second.get("deduplicated") is True, f"the replay was not detected: {second}"
    assert second["ticket_id"] == first["ticket_id"], (
        f"a retry created a second ticket: {first['ticket_id']} then {second['ticket_id']}"
    )


test_repeating_an_identical_write_returns_the_original()
