"""The v1 output contract, asserted field by field."""
...
def test_an_invoice_read_returns_the_documented_shape() -> None:
    text = call_tool("get_invoice_summary", {"cx_id": subject(), "limit": 2})
    payload = json.loads(text)

    assert "invoices" in payload, f"the v1 contract names 'invoices': {sorted(payload)}"
    assert isinstance(payload["invoices"], list), payload
    assert "total_outstanding" in payload, sorted(payload)
    assert "currency" in payload, sorted(payload)
    assert len(payload["invoices"]) <= 2, "the limit argument was ignored"

    for invoice in payload["invoices"]:
        for field in ("invoice_id", "state", "issued_on", "due_on", "total", "outstanding",
                      "currency"):
            assert field in invoice, f"'{field}' missing from InvoiceSummary: {sorted(invoice)}"
        # Money crosses the wire as a decimal string, never a float: a float cannot
        # represent 0.10 exactly, and a billing system that rounds is a billing system
        # that gets sued.
        for money in ("total", "outstanding"):
            assert isinstance(invoice[money], str), (
                f"{money} came back as {type(invoice[money]).__name__}, not a decimal string"
            )
            float(invoice[money])  # parses as a number, but was carried as text

    assert isinstance(payload["total_outstanding"], str), payload["total_outstanding"]


test_an_invoice_read_returns_the_documented_shape()
